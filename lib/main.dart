...
import 'package:flutter/services.dart'; // Required for TextInputFormatter, Clipboard
...
import 'package:url_launcher/url_launcher.dart'; // For launching URLs (download)
import 'package:share_plus/share_plus.dart'; // For sharing capabilities
...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> _loadThemePreference() async {
    ...
    final isDarkMode = prefs.getBool('isDarkMode') ?? false; // Default to false (light)
    setState(() {
      _themeMode = isDarkMode ? ThemeMode.dark : ThemeMode.light;
    });
  }

  // Callback method to update the themeMode from child widgets
  ...
  // Helper to save theme preference
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // Pass the toggleTheme method down to ServerHomePage
...
class ServerHomePage extends StatefulWidget {
  final Function(bool) toggleTheme; // Callback to toggle theme in MyApp
  final ThemeMode initialThemeMode; // Initial theme mode from MyApp
...
class _ServerHomePageState extends State<ServerHomePage> {
  ...
  String? _serverIpAddress; // To store the actual IP the server binds to
  ...
  // Theme state variable for this widget, initialized from MyApp's initialThemeMode
  late bool _isDarkMode;

  @override
  void initState() {
    super.initState();
    // Initialize _isDarkMode based on the initialThemeMode passed from MyApp
    _isDarkMode = widget.initialThemeMode == ThemeMode.dark;

    _initPreferencesAndLoad(); // Load saved preferences (including theme)

    // Add listeners to text fields to save changes as they are typed
  ...
  void dispose() {
    ...
    _stopServer(); // Ensure server is stopped
  ...
  Future<void> _initPreferencesAndLoad() async {
    ...
    await _loadSavedPreferences(); // Await loading of preferences
  }
  ...
  Future<void> _loadSavedPreferences() async {
    ...
    setState(() {
      ...
      final loadedIsDarkMode = _prefs.getBool('isDarkMode') ?? false; // Load theme preference
      // Only update if different to avoid unnecessary rebuilds, but ensure MyApp knows
      if (_isDarkMode != loadedIsDarkMode) {
        _isDarkMode = loadedIsDarkMode;
      }
    });
    // Immediately tell the parent MyApp to update its theme based on the loaded preference
  ...
  Future<void> _savePreference(String key, dynamic value) async {
    if (value is String) {
      ...
      await _prefs.remove(key); // Remove if value is null or unsupported type
    }
  ...
  void _toggleTheme() {
    setState(() {
      _isDarkMode = !_isDarkMode; // Toggle the local state
    });
    _savePreference('isDarkMode', _isDarkMode); // Save the new theme preference
    widget.toggleTheme(_isDarkMode); // Call parent's toggleTheme callback
  ...
  Future<bool> _requestStoragePermission() async {
    ...
    } else if (Platform.isIOS) { // Removed Platform.isMacOS from here
  ...
  Future<void> _pickDirectory() async {
    ...
    try {
      ...
      if (result != null) {
        ...
        _savePreference('selectedDirectoryPath', _selectedDirectoryPath); // Save selected directory
  ...
  Future<void> _pickCustom404File() async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        _savePreference('custom404FilePath', _custom404FilePath); // Save custom 404 file path
      } else {
  ...
  Future<void> _startServer() async {
    ...
    try {
      ...
      setState(() {
        ...
        _serverStatus = 'Server running on http://$_serverIpAddress:$port/' +
    ...
    if (_server != null) {
      ...
      try {
        ...
        setState(() {
          ...
          _statusColor = Colors.red;
          // _custom404FilePath = null; // Removed clearing for persistence
          // _custom404HtmlContent = null; // Removed clearing for persistence
        });
        _server = null;
        _serverIpAddress = null;
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: _toggleTheme, // Calls local _toggleTheme
          ...
          children: <Widget>[
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                filled: true,
                fillColor: Theme.of(context).brightness == Brightness.light ? Colors.grey[100] : Colors.grey[700],
              ),
              keyboardType: TextInputType.text,
              textInputAction: TextInputAction.next,
              style: TextStyle(color: Theme.of(context).textTheme.bodyMedium?.color), // Ensure text color adapts
            ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: Theme.of(context).brightness == Brightness.light ? Colors.blueGrey[50] : Colors.grey[850],
                borderRadius: BorderRadius.circular(8.0),
                border: Border.all(color: Theme.of(context).brightness == Brightness.light ? Colors.blueGrey : Colors.grey[700]!, width: 1.0),
              ),
              child: Text(
                _selectedDirectoryPath != null && _selectedDirectoryPath!.isNotEmpty
            ...
            Container(
              padding: const EdgeInsets.all(12.0),
              decoration: BoxDecoration(
                color: Theme.of(context).brightness == Brightness.light ? Colors.orange[50] : Colors.grey[850],
                borderRadius: BorderRadius.circular(8.0),
                border: Border.all(color: Theme.of(context).brightness == Brightness.light ? Colors.orange : Colors.grey[700]!, width: 1.0),
              ),
              child: Text(
            ...
            Container(
              padding: const EdgeInsets.all(12.0),
              decoration: BoxDecoration(
                color: Theme.of(context).brightness == Brightness.light ? Colors.blue[50] : Colors.grey[850],
                borderRadius: BorderRadius.circular(8.0),
                border: Border.all(color: Theme.of(context).brightness == Brightness.light ? Colors.blue : Colors.grey[700]!, width: 1.0),
              ),
              child: Row(
            ...
            SizedBox(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: Theme.of(context).brightness == Brightness.light ? Colors.blueGrey[50] : Colors.blueGrey[900],
                  borderRadius: BorderRadius.circular(8.0),
                  border: Border.all(color: Theme.of(context).brightness == Brightness.light ? Colors.blueGrey : Colors.grey[700]!, width: 1.0),
                ),
                child: _fileSystemEntries.isEmpty
                  ...
                  child: Text(
                    ...
                    style: TextStyle(color: Theme.of(context).brightness == Brightness.light ? Colors.grey[600] : Colors.white70),
                    textAlign: TextAlign.center,
                  ),
                )
                    : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Icon(
                        ...
                        color: isDirectory ? Colors.amber : (Theme.of(context).brightness == Brightness.light ? Colors.blueGrey[600] : Colors.blueGrey[300]),
                      ),
                      title: Text(
                        filename,
            ...
            SizedBox(
              ...
              child: Container(
                padding: const EdgeInsets.all(8.0),
                decoration: BoxDecoration(
                  color: Theme.of(context).brightness == Brightness.light ? Colors.black.withOpacity(0.8) : Colors.black.withOpacity(0.5),
                  borderRadius: BorderRadius.circular(8.0),
